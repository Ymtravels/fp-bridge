...
(function () {
  ...
  window.addEventListener('message', function (e) {
    ...
    origFetch(url, { credentials: 'include', headers: headers })
      ...
      .then(o => {
        let seats = null, gotFl = false;
        if (o.ok) { try { const j = JSON.parse(o.t); const fl = j.flightList && j.flightList[0]; if (fl) { seats = fl.availableSeats; gotFl = true; } } catch (e) {} }
        window.postMessage({ __ymVerifyRes: { id: v.id, ok: gotFl, seats: seats, status: o.status, hadHeaders: !!siteHeaders, raw: (o.t || '').slice(0, 1400) } }, '*');
      })
